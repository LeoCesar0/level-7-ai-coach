...
export const chatRouter = new Hono()
  ...
  .get(
    ...
    authValidator({ permissionsTo: ROUTE.history.permissions }),
    async (c) => {
  ...
  .post(
    ROUTE.send.path,
    routeValidator({
      schema: ROUTE.send.bodySchema,
    }),
    authValidator({ permissionsTo: ROUTE.send.permissions }),
    async (ctx) => {
      const {
        user: userId,
        message,
